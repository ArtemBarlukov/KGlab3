...
LRESULT CALLBACK InfoWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        CreateWindowEx(0, "STATIC", "Перемещение на стрелочки на клавиатуре.", WS_CHILD | WS_VISIBLE | SS_CENTER,
                       10, 10, 300, 50, hwnd, NULL, NULL, NULL);

        CreateWindowEx(0, "STATIC", "Выполнил Барлуков А.С. ЭВМб-22-1", WS_CHILD | WS_VISIBLE | SS_CENTER,
        10, 40, 300, 20, hwnd, NULL, NULL, NULL);
        // Создаем кнопку "OK"
        CreateWindowEx(0, "BUTTON", "OK", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                       125, 70, 50, 30, hwnd, (HMENU)1, NULL, NULL);
        break;
    }
    case WM_COMMAND:
    {
...
void Init()
{
    Menu_AddButton("INFO", 20, 20, 200, 60, 4);
    Menu_AddButton("JUMP", 20, 90, 200, 60, 4);
    Menu_AddButton("RUN", 20, 160, 200, 60, 4);
    Menu_AddButton("IDLE", 20, 230, 200, 60, 4);
    Menu_AddButton("QUIT", 20, 300, 200, 60, 4);
...
void RenderSpriteAnimation(GLuint texture, float posX, float posY, float width, float height, float scale) {
    float frameWidth = 1.0f / 10; // Предполагается, что numFrames - это количество кадров в спрайт-листе
    ...
    while (!bQuit)
{
    ...
    else
    {
        ...
        glClearColor(0.69f, 0.88f, 0.9f, 1.0f);
        ...
        if (sprite1ButtonPressed)
        {
            ...
            RenderSpriteAnimation(textureSprite1, sprite1PosX, sprite1PosY, 800.0f / 10.0f, 80.0f, 4.0f);
        }
        else if (sprite2ButtonPressed)
        {
            float sprite2PosX = centerX + spritePosX;
            float sprite2PosY = posY + spritePosY;
            RenderSpriteAnimation(textureSprite2, sprite2PosX, sprite2PosY, 800.0f / 10.0f, 80.0f, 4.0f);
        }
        else if (sprite3ButtonPressed)
        {
            float sprite3PosX = centerX + spritePosX;
            float sprite3PosY = posY + spritePosY;
            RenderSpriteAnimation(textureSprite3, sprite3PosX, sprite3PosY, 800.0f / 10.0f, 80.0f, 4.0f);
        }
